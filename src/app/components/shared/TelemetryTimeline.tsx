...
export function TelemetryTimeline({ events }: TelemetryTimelineProps) {
  ...
  return (
    <div className="relative border-l-2 border-ink-100 ml-3 pl-6 space-y-6 pt-1">
      {events.map((event, idx) => {
        ...
        return (
          <div key={event.id || idx} className="relative group flex flex-col gap-1">
            ...
            <div className="absolute -left-[31px] top-4 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-white border border-ink-200">
              {isFulfillment ? (
                <div className="h-2 w-2 rounded-full bg-ok-500" />
              ) : isDispute ? (
            ...
            <div className={`flex flex-col gap-1.5 p-3 rounded-xl transition-all duration-200 ${
              isFulfillment
                ? 'bg-ok-50/50 border border-ok-100/50 shadow-sm'
                : isDispute
                ? 'bg-danger-50/50 border border-danger-100/50 shadow-sm'
                : 'hover:bg-ink-50'
            }`}>
              <div className="flex items-center justify-between gap-4">
                <h4 className={`text-xs font-semibold leading-snug flex items-center gap-1.5 uppercase tracking-wider ${
                  isFulfillment ? 'text-ok-800' : isDispute ? 'text-danger-800' : 'text-ink-800'
                }`}>
                  {isFulfillment && <CheckCircle2 className="h-3.5 w-3.5 text-ok-600 shrink-0" />}
                  {isDispute && <AlertCircle className="h-3.5 w-3.5 text-danger-600 shrink-0" />}
                  <span>{title}</span>
                </h4>
              ...
              <p className={`text-xs leading-relaxed ${
                isFulfillment ? 'text-ok-700/90' : isDispute ? 'text-danger-700/90' : 'text-ink-600'
              }`}>
                {desc}
              </p>
            </div>
